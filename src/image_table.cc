...
#include "singleton.h"
//*****************************************************************************
ImageTable::ImageTable() { delta = singleton::get().DELTA(); }

ImageTable::ImageTable(float _delta) { delta = _delta; }
...
ImageTable::~ImageTable() {
  for (vector<ImageEntry *>::iterator iter = this->images_.begin();
       iter != this->images_.end(); ++iter) {
...
BoundBox ImageTable::CalcBoundBox(Mat image, vector<PixelInfo> joints) {
  ...
  for (vector<PixelInfo>::iterator iter = joints.begin(); iter != joints.end();
       ++iter) {
    int x = iter->x;
    int y = iter->y;
    if (xmax < x) xmax = x;
...
BoundBox ImageTable::CalcBoundBox(Mat image) {
  ...
  binary.convertTo(binary, CV_8UC1, 255.0 / 2000.0,
                   0);  // set 2000 as the white background
  ...
  Scalar color =
      Scalar(rng.uniform(0, 255), rng.uniform(0, 255), rng.uniform(0, 255));
  rectangle(drawing, boundRect[largei].tl(), boundRect[largei].br(), color, 2,
            8, 0);
...
void ImageTable::LoadImages(string Ipath, string Lpath, string Iext) {
  ...
  for (int i = 0; getline(lable, linestr); i++) {
    ...
    for (int r = 0; r < image.rows; ++r) {
      newImage->image_depth[r] = new int[image.cols];
      for (int c = 0; c < image.cols; ++c)
        newImage->image_depth[r][c] = (int)image.at<short>(
            r, c);  // here for uint8 using uchar, for uint16 changing to short.
    }
    ...
    for (int count = 1; count <= 21; count++)  // 21 labels in total
    {
      ...
      iss >> y;
      iss >> z;
      if (count == 1 || count == 7 || count == 14 ||
          count == 21)  // chosen labels
      {
        PixelInfo newJoint;
