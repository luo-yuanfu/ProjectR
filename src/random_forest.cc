...
void RandomForest::PreprocessData(string path) {
  ...
  image_table_->LoadImages(train_path, train_label_path,
                           train_image_ext);  // load images to image table
}
//*****************************************************************************
...
vector<Pixel> RandomForest::SelectInput() {
  ...
  for (int i = 0; i < IMG_SUBSET_SIZE; i++) {
...
//*****************************************************************************
vector<Offset> RandomForest::splitpixel(Tree* tmptree, Node* tmpnode,
                                        Pixel tmppixel,
  ...
  while (tmpnode->left_child_ != NULL) {  // while tmpnode not leaf node
    ...
    if (tmptree->FeatureValue(tmppixel, tmpnode->u_, tmpnode->v_,
                              test_image_table) < tmpnode->threshold_) {
...
// to do:  predict the labels for all the images contained in test_image_table
// using
  ...
  for (int i = 0; i < test_image_table->images_.size(); i++) {
    ...
    for (int j = 0; j < trees_.size(); j++) {
      ...
      if (j == 0) {
        ...
        for (int k = 0; k < 4; k++) {
          images_joints.at(i).at(k).x =
              (images_joints.at(i).at(k).x * j + L.at(k).x) / (j + 1);
          images_joints.at(i).at(k).y =
              (images_joints.at(i).at(k).y * j + L.at(k).y) / (j + 1);
          images_joints.at(i).at(k).depth =
              (images_joints.at(i).at(k).depth * j + L.at(k).depth) / (j + 1);
        }
      }
...
// evaluate the performance by calculating the error. the error now is the
// average
// error over all images and all joints
//*****************************************************************************
double RandomForest::Evaluate(vector<vector<PixelInfo> > predict_result,
                              ImageTable* test_image_table) {
  double error = 0;

  for (int i = 0; i < predict_result.size(); i++) {
    vector<PixelInfo> label_for_one_image =
        test_image_table->images_.at(i)->joints;
    ...
    for (int j = 0; j < label_for_one_image.size(); j++) {
      joints_error +=
          fabs(prediction_of_ith_image.at(j).x - label_for_one_image.at(j).x);
      joints_error +=
          fabs(prediction_of_ith_image.at(j).y - label_for_one_image.at(j).y);
      joints_error += fabs(prediction_of_ith_image.at(j).depth -
                           label_for_one_image.at(j).depth);
    }
    error += joints_error /
             label_for_one_image.size();  // average joint error for one image
  }
