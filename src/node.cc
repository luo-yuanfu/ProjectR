...
//*****************************************************************************
double Node::Entropy() { return Entropy(pixels_); }
//*****************************************************************************
...
// to do: calculate and return the entropy for pixels. say if we want to
// calculate the entropy for parent, then we feed all the pixels to it
// if we want to calculate the entropy for left child, we feed the pixels of the
// left child to it
//*****************************************************************************
double Node::Entropy(const vector<Pixel> pixels) {
  ...
  // Put all joint offsets from all pixels into matrix M, where each row is one
  // pixel
...
double Node::SSE(const vector<Pixel> pixels) {
  ...
  for (int j = 0; j < Njoints; ++j) {
    for (int i = 0; i < Npixels; ++i) {
      sse += (mean_joint[j][0] - pixels[i].label_[j].x) *
             (mean_joint[j][0] - pixels[i].label_[j].x);
      sse += (mean_joint[j][1] - pixels[i].label_[j].y) *
             (mean_joint[j][1] - pixels[i].label_[j].y);
      sse += (mean_joint[j][2] - pixels[i].label_[j].depth) *
             (mean_joint[j][2] - pixels[i].label_[j].depth);
    }
  }
  return sse;
...
// to do: given the pixels in the node, calculate the label of it using
// meanshift method
// the function Tree::CalLeafLabel will call this function to calculate the
// labels
// for all the leaf nodes
//*****************************************************************************
void Node::MeanShift() {
  // does mean shift for all joint locations simultaneously (concat joint labels
  // to Njointx3
  // dimensions)
    ...
    while (1) {
      ...
      for (unsigned int i = 0; i < Npixels; ++i) {
        for (unsigned int j = 0; j < Ndim; ++j) {
          sqDistToAll[i] +=
              (double)(dataPts[i][j] - myMean[j]) * (dataPts[i][j] - myMean[j]);
        }
      }
      vector<int> inInds;
      for (unsigned int i = 0; i < Npixels; ++i) {
...
void Node::Average() {
  ...
  // call set_label() to set label_ for the leaf node after calculating the
  // label
...
// set label_
//*****************************************************************************
void Node::set_label(vector<Offset> label) { label_ = label; }
//*****************************************************************************
...
// set_threshold
//*****************************************************************************
void Node::set_threshold(double threshold) { threshold_ = threshold; }
//*****************************************************************************
