...
int main(int argc, char* argv[]) {
  ...
  test_image_table->LoadImages(test_path, test_label_path,
                               test_image_ext);  // load images to image table

  cout << "finish loading testing images" << endl
       << "testing " << test_image_table->images_.size() << " images" << endl;

  vector<vector<PixelInfo> > predict_result =
      rf_regressor.Predict(test_image_table);

  cout << "Finish testing" << endl;
  t = clock() - t;
  cout << "Test time: " << (double)t / CLOCKS_PER_SEC << endl;

  cout <<endl<< "Begin evaluation: " << endl;
  ...
  if (argc >= 2) {
  	if(argv[1][1]=='p')
  	{
  		cout <<endl<< "the prediction(left) and the true label(right)" << endl;
	    for (int i = 0; i < predict_result.size(); i++) {
	      ...
	      for (int j = 0; j < label_for_one_image.size(); j++) {
	        cout << prediction_of_ith_image.at(j).x << "\t"
	             << label_for_one_image.at(j).x << endl;
	        cout << prediction_of_ith_image.at(j).y << "\t"
	             << label_for_one_image.at(j).y << endl;
	        cout << prediction_of_ith_image.at(j).depth << "\t"
	             << label_for_one_image.at(j).depth << endl;
	      }
	      cout << endl;
	    }
  	}
