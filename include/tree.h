...
class Tree {
 public:
  ...
  double FeatureValue(Pixel piexl, std::pair<int, int> u,
                      std::pair<int, int> v);
  ...
  void ConstructTree();
  void BuildTree(int depth, Node* cur_node);  // build the tree, without
                                              // calculating the labels for the
                                              // leaf nodes
  ...
  double InformationGain(Node* cur_node, vector<Pixel> left_pixel,
                         vector<Pixel> right_pixel);
  double NegativeSSE(Node* cur_node, vector<Pixel> left_pixel,
                     vector<Pixel> right_pixel);
