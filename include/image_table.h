...
class ImageTable {
 private:
  ...
  inline void TransCoord(float &x, float &y, float &z) {
    x = (x * FOCUS / z + Image_Width / 2);
    y = (y * FOCUS / z + Image_Height / 2);
    z = -z;
  };
 ...
 public:
  ...
  void LoadImages(string Ipath, string Lpath,
                  string Iext);   // load image for train images
